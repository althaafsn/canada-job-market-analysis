...
This first step used posting indices to identify sectors and locations worth investigating for a software job search. It established market context; it did not yet measure accessible junior openings, salaries, required skills, or work stress. The initial questions are in [job_search.md](job_search.md).

## Data and method

The notebooks identify the data as the Indeed Job Postings Index, with February 1, 2020 = 100. The folder does not record a source download URL or retrieval date.
...
- [data_analysis.ipynb](data_analysis.ipynb): compares four software/data/IT sectors, then extends the comparison to nine tech/STEM sectors. Uses a 7-day rolling mean for initial sector trends and 14-day means for stock/flow comparisons.
...
![Tech/STEM stock, flow, and ratio comparison](plots/tech_industry_velocity_comparison.png)

![Software, data, and national ratio history; software stock and flow](plots/hiring_velocity_tracking.png)

![National tech-sector correlations with provincial markets](plots/tech_province_correlation.png)

Figures generated by [data_analysis.ipynb](data_analysis.ipynb) and [metro_job_analysis.ipynb](metro_job_analysis.ipynb) are automatically exported and saved directly into the `plots/` directory.
